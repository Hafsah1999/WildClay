...
const pageDetails = [
  ...
  },
  {
    ...
    pagePath: '/Main/ResetPassword'
  },
  {
    pageName: 'product',
    pagePath: '/Main/product'
  },
  {
...
export const VoiceProvider = ({ children }) => {
  ...
  const fillInputUsingVoice = (cb) => {
    if (finalTranscript.toLowerCase().startsWith('shift')) {
      cb();
    }
  }
  ...
  useEffect(() => {
    if (!hasRun.current) {
      ...
      voiceResponse('Welcome to Wild Clay. What are you shopping today?');
  ...
  useEffect(() => {
    ...
    if (finalTranscript.includes('top listening')) {
      ...
      triggerModal('Voice Assistant', 'Tumhare Kahne se chalungi kya??', false, <IconMicrophoneOff size={50} />);
    }
